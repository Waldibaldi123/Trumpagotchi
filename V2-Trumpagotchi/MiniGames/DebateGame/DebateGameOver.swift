...
class DebateGameOver: SKShapeNode {
    ...
    init(debateScene: DebateScene) {
        self.debateScene = debateScene
        super.init()
        let size = debateScene.size
        ...
        scoreLabel = SKLabelNode(text: "Tears collected: \(debateScene.score)")
        scoreLabel.fontName = "AvenirNext-Bold"
        scoreLabel.fontColor = UIColor.white
        scoreLabel.position = CGPoint(x: debateScene.size.width / 2, y: debateScene.size.height / 2)
        addChild(scoreLabel)
        ...
        self.returnButton = SKLabelNode(text: "Give SleepyJoe a break")
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let touch = touches.first {
            ...
            if againButton.contains(location) {
                debateScene.startGame()
            } else if returnButton.contains(location) {
                debateScene.returnToGameSelect()
            }
        }
    }
    
    func loadHighscore() {
        self.highScore = UserDefaults.standard.integer(forKey: "debateHighscore")
    ...
    func saveNewHighscore(newHighscore: Int) {
        ...
        highScoreLabel.text = "Highscore: \(highScore!)"
        UserDefaults.standard.set(highScore, forKey: "debateHighscore")
    }
